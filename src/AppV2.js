...
const App = () => {
    ...
    const [isLargeImageModalOpen, setIsLargeImageModalOpen] = useState(false); // State for large image modal
    const [largeImageUrl, setLargeImageUrl] = useState(''); // State for large image URL
    ...
    try {
        ...
        if (response.data.completed) {
            ...
            // Reset the card state and restart from the first card
            await axios.post(`${apiUrl}/restart_cards`);
            fetchCard(); // Fetch the first card again
        } else {
    ...
    const openLargeImageModal = () => {
        ...
        // Replace 'medium' with 'large' in the image URL
        const largeUrl = card.image_url.replace('medium', 'large');
